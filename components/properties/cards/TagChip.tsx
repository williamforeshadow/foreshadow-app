...
// Read-only list of colored contact-tag pills (the ContactTag analog of TagChips).
  ...
  return (
    <span className={`inline-flex flex-wrap items-center gap-1 ${className}`}>
      {tags.map((t) => (
        <span
          key={t}
          className={`inline-flex items-center px-1.5 py-0.5 text-[10px] font-medium rounded-full border uppercase tracking-[0.04em] ${CONTACT_TAG_CHIP_CLASSES[t]}`}
        >
...
// Multi-select CONTACT tag editor — the ContactTag analog of TagChip. Same
// trigger + checklist UI, driven by CONTACT_TAGS.
        ...
        >
          {CONTACT_TAGS.map((t) => {
            ...
            return (
              ...
              >
                <span
                  className={`inline-block w-2 h-2 rounded-full border ${CONTACT_TAG_CHIP_CLASSES[t]}`}
                />
