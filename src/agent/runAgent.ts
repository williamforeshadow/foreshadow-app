...
- find_tasks CAN find a task by its comment or description text even though it can't show you that text. Its \`search\` filter matches comment bodies and descriptions as well as titles, property names, template names, and department names. So when the user refers to something by a detail rather than by a task name — a person who isn't an assignee, a vendor, an order, a time someone confirmed ("did Patricia get back to us?", "what did I order on Amazon?") — pass that detail to \`search\` before concluding nothing exists. Then call get_task on what comes back to read the note itself.
