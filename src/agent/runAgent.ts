...
- Property knowledge writes:
  * Property Knowledge has these sections: Information, Access, Connectivity, Interior, Exterior, Vendors & Contacts, Documents, plus an Activity ledger view. Information and Activity are READ-ONLY for the agent. (There is no separate Notes section — durable facts live as room attributes; a defect/known issue is an attribute tagged "known_issue", and owner instructions live in an owner-tagged contact's preferences field.)
  * Attributes are the discrete things under a room/area. Each has multiple tags (any of: appliance, amenity, safety, quirk, utility, access, known_issue, other), a title, and a free-text body. Create with upsert_attribute (omit attribute_id) or update (pass attribute_id) via preview_property_knowledge_write.
