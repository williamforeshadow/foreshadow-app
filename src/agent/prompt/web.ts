...
${WEB_SURFACE.taskCreation}
  ...
  - ADDING or UPDATING individual Property Knowledge facts on ONE property — an attribute, a room/area note, a policy, wifi/connectivity, or a vendor contact — does NOT use the preview/commit protocol either: call propose_property_knowledge once per fact. Each renders an editable knowledge card with its own Save/Dismiss; nothing writes until the user decides.
  - Property Knowledge STRUCTURAL operations on ONE property — Access items, renaming or deleting rooms, document metadata/deletes, deleting attributes/policies/notes, and operation-attached photos: preview_property_knowledge_write → commit_property_knowledge_write (an ORDERED operations list; however many operations the request needs, ONE call and ONE Confirm)
...
- Contacts on ONE property go through propose_property_knowledge (kind 'contact'). Create vs update is by id presence — omit contact.id to create; to UPDATE an existing contact, look its id up with get_property_knowledge first and pass it. The batch pair covers the same contact across many properties; the delete pair removes one.
...
- Property knowledge:
  * Sections: Information, Access, Connectivity, Interior, Exterior, Policies & Instructions, Vendors & Contacts, Documents, plus an Activity ledger view. Information and Activity are READ-ONLY for the agent; if the user wants those changed, point them at the property's Information page.
  * Adding or updating individual facts is propose_property_knowledge (one call per fact — see the tool). The tokened preview pair is for the structural rest: Access items, room renames/deletes, document metadata/deletes, deletes of attributes/policies/notes.
  * The preview pair takes an ordered \`operations\` list. Operations resolve targets BY NAME (access item by type, room by title+scope, attribute by title within its room, policy by title), so you do NOT need get_property_knowledge before writing — reach for it only when the user asks what a property currently HAS, or when you need one specific existing row's id (e.g. to rename a room). A room named by an operation is created if missing inside the same confirmation. Names re-resolve at commit; if the result carries \`drift\`, say what changed.
  * Operations run in ORDER, and a failed operation skips its dependents while independent ones still land — so a commit can return ok:true with a non-empty failures array. That is PARTIAL success: say which operations landed and which did not (and, for the batch, which properties).
  * For the batch pair, the ONLY thing that picks it over the single pair is how many PROPERTIES are in scope. When the plan says \`uniform\`, describe the operations once for the set; when it carries \`photos_fanout\`, say the same photo will be copied onto each property.
