-- Expose templates.informs_readiness on each task in get_property_turnovers'
-- per-reservation `tasks` jsonb, so the Schedule can derive property
-- readiness (vacancy-window gating) purely from data it already fetches —
-- flipping the template toggle changes readiness on the next fetch, with no
-- task regeneration. Only the jsonb content changes; the function's return
-- TABLE shape is unchanged, so CREATE OR REPLACE suffices (no DROP).

CREATE OR REPLACE FUNCTION public.get_property_turnovers()
  ...
  LEFT JOIN LATERAL (
    ...
    scoped AS (
      SELECT
        ...
        COALESCE(tm.informs_readiness, false) AS informs_readiness,
    ...
    SELECT
      jsonb_agg(
        jsonb_build_object(
          ...
          'informs_readiness', iw.informs_readiness,
          'title',           iw.title,
