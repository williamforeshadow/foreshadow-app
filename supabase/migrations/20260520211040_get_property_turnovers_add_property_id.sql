-- Add property_id to get_property_turnovers: at the card level (from
-- reservations.property_id) and inside each task object. The task
-- detail panel's scheduled-date picker needs property_id to fetch
-- reservations; the Turnovers page was the only surface still
-- starved of it because this RPC never selected the column.
-- Return-type shape changes, so the function must be dropped first.
DROP FUNCTION IF EXISTS public.get_property_turnovers();

CREATE FUNCTION public.get_property_turnovers()
 RETURNS TABLE(id uuid, property_id uuid, property_name text, guest_name text, check_in timestamp with time zone, check_out timestamp with time zone, next_check_in timestamp with time zone, occupancy_status text, tasks jsonb, total_tasks integer, completed_tasks integer, tasks_in_progress integer, turnover_status text)
 LANGUAGE plpgsql
AS $function$
DECLARE
